...
<body id="manage">
 ...
 <?php
    ...
    if ($item){
			...
			if ($item==="headings"){
				$sql = "SELECT * FROM ".$table."_topics ORDER BY heading"; //headings are in the topics table
			} else {
			...
			switch ($item) {
				...
				case 'topics':
					$sql .= " ORDER BY heading, topic"; //numbered order
					break;
			}
			$error = null;
    ...
    if ($result->num_rows > 0 && $item!=="headings") {
        while($row = $result->fetch_assoc()) {
            if ($row['id']!=="0"){
                ...
                $str.='<tr id="tableRow'.$row['id'].'" data-id="'.$row['id'].'" class="'.$item.'Row">';
								...
								if ($item==="topics"){ //managing topics
									...
									switch($row['id']){
										...
										default:
											...
											//$str.='<td class="col3"><input type="color" class="ta hex" id="colour'.$row['id'].'" data-field="colour" data-id="'.$row['id'].'" onblur="inputColorBlur(this);" value="test"></td>';
											$str.='<td class="col3"><input type="color" id="colour'.$row['id'].'" data-field="colour" data-id="'.$row['id'].'"  onblur="inputColorBlur(this);" value="#'.$row['colour'].'"></td>';
