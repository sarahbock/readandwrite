...
function inputColorBlur(e){
    //show("new text "+$(e).html());
   // if ($(e).html()!==initialText){ //only save it if it's different to the intiial text
   //category 14 = CC79A7
    var id=$(e).data("id");
    var field=$(e).data("field");
    var value=cleanUp($(e).val());
    $(e).closest('tr').css("background-color","#"+$(e).html()); //apply new hex code to table row
    saveField(id,field,value);
    console.log('saveField',id,field,value);
}
...
function setFilter(n){
    ...
    if (n===1){ //first level filter selected
        switch (val){
            ...
            case "missingaudio": reloadPage(val); break;
            case "missingimage": reloadPage(val); break;
...
function getSecondFilterValues(filter,keyword){
    ...
    $("#filter2").html(filterSelectStr).css("display", "flex");
...
function getFilters(){
	...
	.done(function() {
        $("#filter1").css("display","flex"); //show filter option once filters have loaded
...
function showMode(n){
    resetMode();
    $("#modeButton"+n+" .modeButtonFull").css("display","flex");
    $("#modeButton"+n+"Close").css("display","flex");
    showModeView(n);
    mode=n;
    if (mode==="Quick"){
...
function addTopic(headingId, count){
  console.log(apiPath+"set-data-temp.php?id=0&field=topics&table="+language1+"_topics&value="+headingId+"&count="+count);
  $.get(apiPath+"set-data-temp.php?id=0&field=topics&table="+language1+"_topics&value="+headingId+"&count="+count, function() {})//write to db
  .done(function(){
    window.location.reload();
  });
}
...
function addHeading(count){
  console.log(apiPath+"set-data-temp.php?id=0&field=headings&table="+language1+"_headings&value=&count="+count);
  $.get(apiPath+"set-data-temp.php?id=0&field=headings&table="+language1+"_headings&value=&count="+count, function() {})//write to db
  .done(function(){
    window.location.reload();
  });
}
...
function saveField(id,field,value){
    ...
    if($("body").attr("id")==="entries" || $("body").attr("id")==="edit"){
      ...
      if ( field==='heading' ) {
        show("SAVE FIELD "+apiPath+"set-item.php?id="+id+"&field=heading&table="+language1+"&value="+value+"&item=headings&replace=");
        $.get(apiPath+"set-item.php?id="+id+"&field=heading&table="+language1+"&value="+value+"&item=headings&replace=", function() {})
    		.done(function() {confirmSave(true);})
    		.fail(function() { });
      } else if ( field === 'topic'){
        show("SAVE FIELD "+apiPath+"set-item.php?id="+id+"&field=topic&table="+language1+"&value="+value+"&item=topics&replace=");
...
function addRow(id){
    ...
    $.get(apiPath+"set-data.php?id=0&table="+language1, function(data) {
        ...
        str+='<td class="col2"><div contenteditable="true" class="ta glossing" data-field="translation" data-id="'+lastRow+'" id="translation'+lastRow+'" onfocus="textAreaFocus(this);" onblur="textAreaBlur(this);" onclick="showQuick()"></div></td>';
        /*Creole TEXT (UMPILA ONLY)*/
        if (language1==='umpila') str+='<td class="col2"><div contenteditable="true" class="ta glossing" data-field="meaning" data-id="'+lastRow+'" id="meaning'+lastRow+'" onfocus="textAreaFocus(this);" onblur="textAreaBlur(this);" onclick="showQuick()"></div></td>';
